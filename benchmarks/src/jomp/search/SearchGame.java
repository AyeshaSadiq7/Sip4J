  ...
  int ab(int alpha, int beta)
  {
    ...
    for (i = nav = 0; ++i <= 7;) {
      if ((h = height[i]) <= 6) { // height = r
        ...
        if (!(h+1 <= 6 && wins(i,h+1,1<<otherside))) //
    	//Vertex Name = rows, Post Permissions = share, Pre-Permissions =share // It should be pure
    	//Vertex Name = dias, Post Permissions = share, Pre-Permissions =share // it should be pure
        av[nav++] = i;
      }
    }
    if (nav == 0)
