...
public class Knapsack {
	...
	public static Indiv createRandomIndiv(Indiv ind) {	
		...
		for (int i = 0; i < cromSize; i++) {//pure(cromSize)
			...
			boolean b = (Knapsack.rand.nextDouble() < 0.01); //full(this)
			
			ind.set(i, b); //full(this)
			
			hasSth = hasSth || b;
		}
		if (!hasSth) { // Enforce at least 1 item in backpack;
			ind.set(rand.nextInt(cromSize), true);//full(this)
		}
		return ind;
	}
	
	/*@Perm(requires="full(this) in alive in alive",
			ensures="full(this) in alive in alive")*/
	...
	public static void evaluate(Indiv indiv) {
		...
		int[] ph = phenotype(indiv);//full(this) in alive * unique(result)
	...
	public static void mutate(Indiv indiv) {
		if (rand.nextFloat() < Knapsack.prob_mut) {//full(this) in alive
			int p = rand.nextInt(cromSize);// full(this) * pure(#0) in alive
			indiv.set(p,!indiv.has[p]); //full(this)
		}
	}
