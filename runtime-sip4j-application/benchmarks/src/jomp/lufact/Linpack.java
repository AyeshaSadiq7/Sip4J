...
*/
 ...
 /* Method Name = dgefa
  ...
  final int dgefa( double a[][], int lda, int n, int ipvt[])
  {
	...
	l = idamax(col_k,n,k,1) + k; // col_k = a = r
	...
	if (col_k[l] != 0) {
	  ...
	  dscal(col_k,n,t,kp1,1);  //col_k = a= rw
  ...
  final void dgesl( double a[][], int lda, int n, int ipvt[], double b[], int job)
  {
   ...
   if (nm1 >= 1) {
	for (k = 0; k < nm1; k++) {
	  ...
	  daxpy(a[k],n,b,t,kp1,1,kp1,1); //a[] is passed here a = r, b = rw
	...
	t = -b[k];
	daxpy(a[k],k,b,t,0,1,0,1);//a = r and b = rw
      }
    }
    else {
      ...
      for (k = 0; k < n; k++) {
    	  t = ddot(a[k],b,k,0,1,0,1);// a = r and b = r
	...
	//  b[k] += ddot(n-(kp1),a[k],kp1,1,b,kp1,1);// a = r, b = r
	  b[k] += ddot(a[k],b,n,kp1,1,kp1,1);// a = r, b = r
  ...
  */
  ...
  final void daxpy(double dx[], int n, double dy[], double da, int dx_off, int incx,
	      int dy_off, int incy)
  {
    int i,ix,iy;
  ...
  */
  ...
  final double ddot(double dx[], double dy[], int n, int dx_off, int incx, 
	       int dy_off, int incy)
  {
    double dtemp;
    int i,ix,iy;
