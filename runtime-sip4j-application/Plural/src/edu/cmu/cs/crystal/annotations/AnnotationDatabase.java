...
/*
	...
	public List<ICrystalAnnotation>  getAnnosForType(IType type) throws JavaModelException {
		...
		if (result == null) {
			result = createAnnotationsOverload1(type.getAnnotations(), type);
			classes.put(name, result);
		}
		return result;
	}
	
	public List<ICrystalAnnotation> getAnnosForTypeOverload(ITypeBinding type) {
	...
	protected List<ICrystalAnnotation> createAnnotations(IAnnotationBinding[] bindings) {
		...
		for (IAnnotationBinding anno : bindings) {
			result.addAll(createAnnotationsOverload3(anno));
		}
		return Collections.unmodifiableList(result);
	}

	protected List<ICrystalAnnotation> createAnnotationsOverload1(IAnnotation[] annotations, IType relative_type) throws JavaModelException {
		List<ICrystalAnnotation> result = new ArrayList<ICrystalAnnotation>(annotations.length);
		for (IAnnotation anno : annotations) {
			result.addAll(createAnnotationsOverload2(anno, relative_type));
		}
		return Collections.unmodifiableList(result);
	}
	

	protected List<ICrystalAnnotation> createAnnotationsOverload2(IAnnotation anno, IType relative_type) throws JavaModelException {
		if (isMulti(anno, relative_type)) {
			for (IMemberValuePair pair : anno.getMemberValuePairs()) {
				...
				if (value instanceof Object[]) {
					...
					return Collections.singletonList(createAnnotationOverload5((IAnnotationBinding) value));
				}
			}
	...
	protected List<ICrystalAnnotation> createAnnotationsOverload3(IAnnotationBinding binding) {
		if (isMulti(binding)) {
			for (IMemberValuePairBinding pair : binding.getAllMemberValuePairs()) {
				...
				if (value instanceof Object[]) {
					...
					List<ICrystalAnnotation> result =
					    new ArrayList<ICrystalAnnotation>(array.length);
					for (Object o : array) {
						result.add(createAnnotationOverload5((IAnnotationBinding) o));
					}
					return Collections.unmodifiableList(result);
				}
				else {
		...
		return Collections.singletonList(createAnnotationOverload5(binding));
	}
*/
	...
	public ICrystalAnnotation createCrystalAnnotation(IType typeOfAnnotation) throws JavaModelException {
		...
		if (annoClass == null) {
			...
			for (int ndx = 0; ndx < metas.length && annoClass == null; ndx++) {
				...
				Pair<String,String> meta_name = null;//getQualifiedAnnoType(meta, typeOfAnnotation);
