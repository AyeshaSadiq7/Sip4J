...
public abstract class AbstractTACBranchSensitiveTransferFunction<LE extends LatticeElement<LE>> 
	...
	public IResult<LE> transferOver2(ArrayInitInstruction instr, List<ILabel> labels,
			LE value) {
		return LabeledSingleResult.createResult(value, labels);
	}

	public IResult<LE> transferOver4(BinaryOperation binop, List<ILabel> labels,
			LE value) {
		return LabeledSingleResult.createResult(value, labels);
	}

	public IResult<LE> transferO1(CastInstruction instr, List<ILabel> labels,
			LE value) {
		return LabeledSingleResult.createResult(value, labels);
	}

	public IResult<LE> transferOverload4(DotClassInstruction instr, List<ILabel> labels,
			LE value) {
		return LabeledSingleResult.createResult(value, labels);
	}
